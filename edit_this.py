...
class Controller():
    ...
    def planning(self, use_firmware, initial_info):
        ...
        self.banking = False
        roll_angle = np.radians(30)  # degrees
        self.ascend_with_angle = False
        
        avg_desired_velocity = 1.1  # m/s
        ...
        if self.ascend_with_angle:
            ...
            self.takeoff_time = self.post_takeoff_height / 0.5  # Time to reach the post-takeoff height
        ...
        t = h.get_path(start, end, gate_rad, obs_rad, self.NOMINAL_GATES, self.NOMINAL_OBSTACLES)
        ...
        if self.ascend_with_angle:
            ascend_time = (gate_height - self.post_takeoff_height) / 0.6  # Time to reach the gate height
        ...
        for dimension in range(3):
            if self.banking:
                tck = splrep(x=t_waypoints, y=waypoints[dimension], s=0.05)
            else:
...
def calculate_roll_rate(ref_x, ref_y, ref_z, ref_vx, ref_vy, ref_vz, dt, g=9.81, target_roll_angle=np.radians(45), lookahead=50):
    ...
    for i in range(1, len(ref_x) - 1):
        ...
        # Compute roll angle
        roll_angle = 0.5*np.arctan(a_y / g)
        ...
        if i > 1: #and a_y>0.6:
            ...
            max_rate = np.radians(45)
            roll_rate = min((roll_angle - prev_roll_angle) / dt, max_rate)
    ...
    # max_rate = np.radians(45)
    # roll_rates = np.clip(roll_rates, -max_rate, max_rate)
