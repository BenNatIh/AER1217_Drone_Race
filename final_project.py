...
from project_utils import Command, thrusts, plot_trajectory, draw_trajectory
from edit_this_straightLine import Controller
...
def run(test=False):
    ...
    # Plot and show the trajectory error
    print("Plotting actual trajectory taken...")
    draw_trajectory(old_info, ctrl.waypoints, ctrl.act_x, ctrl.act_y, ctrl.act_z, error=True)

    if collisions_count == 0:
        print("Plotting trajectory error...")
        plot_trajectory(np.linspace(0, 1, int(len(ctrl.act_x/ctrl.CTRL_FREQ))), ctrl.waypoints, 
                        ctrl.act_x, ctrl.act_y, ctrl.act_z, wait=10)
        error_x = np.subtract(ctrl.ref_x,ctrl.act_x)
        error_y = np.subtract(ctrl.ref_y,ctrl.act_y)
        error_z = np.subtract(ctrl.ref_z,ctrl.act_z)
        plot_trajectory(np.linspace(0, 1, int(len(ctrl.ref_x/ctrl.CTRL_FREQ))), ctrl.waypoints, 
                        error_x, error_y, error_z, wait=10, threeD=False)
